...
async def aggregate_from_multiple_sources() -> Dict[str, Any]:
    ...
    sources = [
        ...
        },
        {
            ...
            "params": {"filter": "active", "max_results": 1000},
    ...
    async def fetch_and_transform(source: Dict[str, Any], index: int) -> Dict[str, Any]:
        ...
        try:
            ...
            print(
                f"✓ [{index + 1}/{len(sources)}] {source['name']}: {len(raw_data)} records ({duration:.0f}ms)"
            )

            # Transform to common format
            transformed = (
                source["transform"](raw_data) if source.get("transform") else raw_data
            )
            ...
            duration = (datetime.now() - source_start).total_seconds() * 1000
            print(
                f"✗ [{index + 1}/{len(sources)}] {source['name']} failed: {error} ({duration:.0f}ms)"
            )
    ...
    stats = {
        ...
        "oldest_record": (
            min(unique_list, key=lambda x: x["joined"])["joined"].isoformat()
            if unique_list
            else None
        ),
        "newest_record": (
            max(unique_list, key=lambda x: x["joined"])["joined"].isoformat()
            if unique_list
            else None
        ),
    }
...
async def aggregate_metrics() -> Dict[str, Any]:
    ...
    web_analytics, mobile_analytics, server_metrics = await asyncio.gather(
        call_mcp_tool(
            "mcp__analytics__get_web_metrics",
            {
                "period": "last_7_days",
        ...
        call_mcp_tool(
            "mcp__analytics__get_mobile_metrics",
            {
                "period": "last_7_days",
        ...
        call_mcp_tool(
            "mcp__monitoring__get_server_metrics",
            {"period": "last_7_days", "metrics": ["cpu", "memory", "requests"]},
        ),
    )
    ...
    await call_mcp_tool(
        "mcp__database__insert", {"table": "aggregated_metrics", "record": combined}
    )
...
async def join_data_sources() -> List[Dict[str, Any]]:
    ...
    users, orders, preferences = await asyncio.gather(
        ...
        call_mcp_tool(
            "mcp__database__query",
            {"table": "orders", "fields": ["id", "user_id", "total", "created_at"]},
        ),
        call_mcp_tool(
            "mcp__database__query",
            {
    ...
    print(
        f"✓ Fetched: {len(users)} users, {len(orders)} orders, {len(preferences)} preferences"
    )
