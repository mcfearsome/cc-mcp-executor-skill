...
async def process_files(
    ...
    file_list = await call_mcp_tool('mcp__filesystem__list_directory', {
        'path': directory_path,
        'recursive': False
    })
    ...
    async def get_file_info(item: Dict[str, Any]) -> FileInfo:
        stat = await call_mcp_tool('mcp__filesystem__stat', {
            'path': item['path']
        })
    ...
    files_with_metadata = await asyncio.gather(*[
        get_file_info(item) for item in file_list
    ...
    if parallel:
        ...
        results = await asyncio.gather(*[
            process_file(file) for file in filtered
        ])
    else:
