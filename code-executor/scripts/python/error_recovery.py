...
async def fetch_data_with_recovery(data_id: str) -> RecoveryResult:
    ...
    try:
        data = await call_with_retry(
            'mcp__primary__get_data',
            {'id': data_id, 'timeout': 5000},
            retry_config
        )
    ...
    try:
        attempts += 1
        data = await call_mcp_tool('mcp__secondary__get_data', {
            'id': data_id
        })
    ...
    try:
        attempts += 1
        cached = await call_mcp_tool('mcp__cache__get', {
            'key': f'data:{data_id}'
        })
...
batch_result = await batch_process_with_recovery([
    'user-123',
    'user-456',
    'user-789'
])
