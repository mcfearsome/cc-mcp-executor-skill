 ...
 */
function transformSourceA(data: unknown[]): unknown[] {
 ...
 */
function transformSourceB(data: unknown[]): unknown[] {
  return data.map((item) => ({
    id: item.id.toString(),
 ...
 */
function transformSourceC(data: unknown[]): unknown[] {
  return data.map((item) => {
    ...
    return {
      ...
      joined: new Date((rec.signup_timestamp as number) * 1000),
      source: "source-c",
      metadata: {
        originalId: rec.uid,
...
async function aggregateFromMultipleSources(): Promise<AggregatedData> {
  ...
  const results = await Promise.allSettled(
    sources.map(async (source, index) => {
      ...
      console.log(
        `[${index + 1}/${sources.length}] Fetching from ${source.name}...`,
      );
      ...
      try {
        ...
        console.log(
          `✓ [${
            index + 1
          }/${sources.length}] ${source.name}: ${rawData.length} records (${duration}ms)`,
        );
  ...
  const successful = results
    .filter((
      r,
    ): r is PromiseFulfilledResult<{ success: boolean; value: unknown }> =>
  ...
  const _stats = {
    ...
    oldestRecord: uniqueData.reduce(
      (oldest, item) => item.joined < oldest.joined ? item : oldest,
      uniqueData[0],
    ),
    newestRecord: uniqueData.reduce(
      (newest, item) => item.joined > newest.joined ? item : newest,
      uniqueData[0],
    ),
  };
  ...
  console.log(
    `Records: ${uniqueData.length} unique from ${allData.length} total`,
  );
 ...
 */
async function aggregateMetrics(): Promise<unknown> {
  ...
  const combined = {
    ...
    timestamp: new Date().toISOString(),
    web: {
      totalPageviews: webAnalytics.pageviews.reduce(
        (a: number, b: number) => a + b,
        0,
      ),
      avgSessions:
        webAnalytics.sessions.reduce((a: number, b: number) => a + b, 0) / 7,
      bounceRate:
        webAnalytics.bounceRate.reduce((a: number, b: number) => a + b, 0) / 7,
    },
    mobile: {
      totalAppOpens: mobileAnalytics.appOpens.reduce(
        (a: number, b: number) => a + b,
        0,
      ),
      avgActiveUsers:
        mobileAnalytics.activeUsers.reduce((a: number, b: number) => a + b, 0) /
        7,
      crashRate:
        mobileAnalytics.crashRate.reduce((a: number, b: number) => a + b, 0) /
        7,
    },
    server: {
      avgCpu: serverMetrics.cpu.reduce((a: number, b: number) => a + b, 0) / 7,
      avgMemory:
        serverMetrics.memory.reduce((a: number, b: number) => a + b, 0) / 7,
      totalRequests: serverMetrics.requests.reduce(
        (a: number, b: number) => a + b,
        0,
      ),
    },
 ...
 */
async function joinDataSources(): Promise<unknown> {
  ...
  console.log(
    `✓ Fetched: ${users.length} users, ${orders.length} orders, ${preferences.length} preferences`,
  );

  // Create lookup maps for efficient joining
  const prefsMap = new Map(
    preferences.map((p: Record<string, unknown>) => [p.user_id, p]),
  );
  const ordersMap = new Map<string, unknown[]>();
  ...
  const enrichedUsers = users.map((user: Record<string, unknown>) => {
    ...
    return {
      ...
      orders: {
        ...
        total: userOrders.reduce(
          (sum: number, o) =>
            sum + ((o as Record<string, unknown>).total as number),
          0,
        ),
